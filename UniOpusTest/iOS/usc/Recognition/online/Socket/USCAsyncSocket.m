...
#ifndef USCAsyncSocketLoggingEnabled
#define USCAsyncSocketLoggingEnabled 0
#endif

#if USCAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext USCAsyncSocketLoggingContext
...
#ifndef USCAsyncSocketLogLevel
#define USCAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = USCAsyncSocketLogLevel;
...
NSString *const USCAsyncSocketException = @"USCAsyncSocketException";
NSString *const USCAsyncSocketErrorDomain = @"USCAsyncSocketErrorDomain";

NSString *const USCAsyncSocketQueueName = @"USCAsyncSocket";
NSString *const USCAsyncSocketThreadName = @"USCAsyncSocket-CFStream";

NSString *const USCAsyncSocketManuallyEvaluateTrust = @"USCAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const USCAsyncSocketUseCFStreamForTLS = @"USCAsyncSocketUseCFStreamForTLS";
#endif
NSString *const USCAsyncSocketSSLPeerID = @"USCAsyncSocketSSLPeerID";
NSString *const USCAsyncSocketSSLProtocolVersionMin = @"USCAsyncSocketSSLProtocolVersionMin";
NSString *const USCAsyncSocketSSLProtocolVersionMax = @"USCAsyncSocketSSLProtocolVersionMax";
NSString *const USCAsyncSocketSSLSessionOptionFalseStart = @"USCAsyncSocketSSLSessionOptionFalseStart";
NSString *const USCAsyncSocketSSLSessionOptionSendOneByteRecord = @"USCAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const USCAsyncSocketSSLCipherSuites = @"USCAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const USCAsyncSocketSSLDiffieHellmanParameters = @"USCAsyncSocketSSLDiffieHellmanParameters";
#endif

enum USCAsyncSocketFlags
{
...
@interface USCAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation USCAsyncSocketPreBuffer
...
/**
 * The USCAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface USCAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(USCAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation USCAsyncReadPacket
...
/**
 * The USCAsyncWritePacket encompasses the instructions for any given write.
**/
@interface USCAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation USCAsyncWritePacket
...
/**
 * The USCAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface USCAsyncSpecialPacket : NSObject
{
  @public
...
@implementation USCAsyncSpecialPacket
...
@implementation USCAsyncSocket
{
	...
	USCAsyncReadPacket *currentRead;
	USCAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	USCAsyncSocketPreBuffer *preBuffer;
	...
	USCAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([USCAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[USCAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak USCAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong USCAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create USCAsyncSocket instance for accepted socket
			
			USCAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [USCAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [USCAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [USCAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"USCAsyncSocketConnectTimeoutError",
	                                                     @"USCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"USCAsyncSocketReadMaxedOutError",
														 @"USCAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"USCAsyncSocketReadTimeoutError",
	                                                     @"USCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"USCAsyncSocketWriteTimeoutError",
	                                                     @"USCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"USCAsyncSocketClosedError",
	                                                     @"USCAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:USCAsyncSocketErrorDomain code:USCAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the USCAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	USCAsyncReadPacket *packet = [[USCAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[USCAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[USCAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued USCAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued USCAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via USCAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		USCAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		USCAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	USCAsyncWritePacket *packet = [[USCAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[USCAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[USCAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued USCAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued USCAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a USCAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		USCAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	USCAsyncSpecialPacket *packet = [[USCAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			USCAsyncSpecialPacket *tlsPacket = (USCAsyncSpecialPacket *)currentRead;
			NSDictionary *tlsSettings = tlsPacket->tlsSettings;
			
			NSNumber *value = [tlsSettings objectForKey:USCAsyncSocketUseCFStreamForTLS];
...
{
	USCAsyncSocket *asyncSocket = (__bridge USCAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:USCAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. USCAsyncSocketSSLPeerID
	//  4. USCAsyncSocketSSLProtocolVersionMin
	//  5. USCAsyncSocketSSLProtocolVersionMax
	//  6. USCAsyncSocketSSLSessionOptionFalseStart
	//  7. USCAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. USCAsyncSocketSSLCipherSuites
	//  9. USCAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:USCAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. USCAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:USCAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. USCAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:USCAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. USCAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:USCAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. USCAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:USCAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. USCAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:USCAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. USCAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:USCAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for USCAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for USCAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use USCAsyncSocketSSLProtocolVersionMin & USCAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[USCAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"USCAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[USCAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[USCAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("USCAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
+ (void)stopCFStreamThreadIfNeeded
{
	...
	#pragma clang diagnostic warning "-Wimplicit-retain-self"
		...
		if (--cfstreamThreadRetainCount == 0)
		{
			...
			// wake up the thread
			[USCAsyncSocket performSelector:@selector(ignore:)
...
{
	[[NSThread currentThread] setName:USCAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(USCAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(USCAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	USCAsyncSocket *asyncSocket = (__bridge USCAsyncSocket *)pInfo;
