...
// ** 使用开源socket连接类，为避免和应用层发生类名冲突，将GCDAsyncSocket改成了USCAsyncSocket **
...
@class USCAsyncReadPacket;
@class USCAsyncWritePacket;
@class USCAsyncSocketPreBuffer;

extern NSString *const USCAsyncSocketException;
extern NSString *const USCAsyncSocketErrorDomain;

extern NSString *const USCAsyncSocketQueueName;
extern NSString *const USCAsyncSocketThreadName;

extern NSString *const USCAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const USCAsyncSocketUseCFStreamForTLS;
#endif
#define USCAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define USCAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define USCAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const USCAsyncSocketSSLPeerID;
extern NSString *const USCAsyncSocketSSLProtocolVersionMin;
extern NSString *const USCAsyncSocketSSLProtocolVersionMax;
extern NSString *const USCAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const USCAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const USCAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const USCAsyncSocketSSLDiffieHellmanParameters;
#endif

#define USCAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, USCAsyncSocketError) {
	USCAsyncSocketNoError = 0,           // Never used
	USCAsyncSocketBadConfigError,        // Invalid configuration
	USCAsyncSocketBadParamError,         // Invalid parameter was passed
	USCAsyncSocketConnectTimeoutError,   // A connect operation timed out
	USCAsyncSocketReadTimeoutError,      // A read operation timed out
	USCAsyncSocketWriteTimeoutError,     // A write operation timed out
	USCAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	USCAsyncSocketClosedError,           // The remote peer closed the connection
	USCAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface USCAsyncSocket : NSObject

/**
 * USCAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, USCAsyncSocket will automatically create it's own socket queue.
 ...
 * 
 * For accepting incoming connections, this means USCAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means USCAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, USCAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, USCAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of USCAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a USCAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a USCAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that USCAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - USCAsyncSocketManuallyEvaluateTrust
 ...
 *     So USCAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - USCAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default USCAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct USCAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, USCAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by USCAsyncSocket,
 ...
 *
 * - USCAsyncSocketSSLProtocolVersionMin
 * - USCAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - USCAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see USCAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use USCAsyncSocketSSLProtocolVersionMin & USCAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * USCAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(USCAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(USCAsyncSocket *)sock didAcceptNewSocket:(USCAsyncSocket *)newSocket
 * {
...
@protocol USCAsyncSocketDelegate
@optional
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(USCAsyncSocket *)sock didConnectToUrl:(NSURL *)url;
 ...
 * 
 * Note: If the USCAsyncSocket instance is deallocated while it is still connected,
 ...
 * - USCAsyncSocketManuallyEvaluateTrust == YES
 *
